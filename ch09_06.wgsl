...
fn sdf(p: vec3<f32>) -> f32 {
    // return box_sdf(fract(p + 0.5) - 0.5, vec3(0.0), vec3(0.2), 0.0);
    return octahedron_sdf((fract(2.0 * p + 0.5) - 0.5) * (1.0 / (2. + 0.2 * f32(globals.time))), 0.1);
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let camera_pos = euler(vec3( 0.0,  0.0,  4.0 + 1.2 * sin(globals.time * 0.2)), t);
    ...
    for (var i = 0; i < 50; i++) {
        ...
        if sdf(r) < 0.001 {
            ...
            let diff = pow(0.9 * max(dot(light_dir, grad_sdf(r)), 0.0), 0.8);

            color = vec3(0.40, 0.88, 0.98) * (diff + ambient_light);
        }
