...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let camera_pos = euler(vec3( 0.0,  0.0,  2.0), 6.6 * sin(t));
    let camera_dir = euler(vec3( 0.0,  0.0, -1.0), 6.6 * sin(t));
    let camera_up  = euler(vec3( 0.0,  1.0,  0.0), 6.6 * sin(t));
    let light_dir  = euler(vec3( 0.0,  0.0,  1.0), 6.6 * sin(t));
    ...
    for (var i = 0; i < 50; i++) {
        ...
        if sdf(r) < 0.001 {
            ...
            let f = tex(1.2 * r.xy + 1.2 * r.yz, (1.2 + 1.5 * sin(0.12 * globals.time)));
            let diff = pow((0.2 + 0.9 * f) * max(dot(light_dir, grad_sdf(r)), 0.0), -1.2);
            color = mix(vec3(0.32, 0.42, 0.98), vec3(0.01, 0.0, 0.03), f) * (diff + ambient_light);
            break;
        }
