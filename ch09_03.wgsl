...
fn sdf(p: vec3<f32>) -> f32 {
    let d1 = sphere_sdf(p, vec3(0.2 * cos(globals.time * 1.3), 0.2 * sin(globals.time * 1.4), 0.0), 0.3);
    var d2 = 1000.;

    for (var i = 0.0; i < 2.0 * PI; i += PI / 3.0) {
        d2 = min(d2, sphere_sdf(p, vec3(cos(i), sin(i), 0.0), 0.3));
    }

    return smin(d1, d2, 0.5 + 0.5 * sin(globals.time)) + 0.03 * pnoise31(10.0 * p);
}
