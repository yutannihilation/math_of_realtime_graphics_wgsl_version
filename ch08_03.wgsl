...
fn sphare_sdf(p: vec3<f32>, c: vec3<f32>, r: f32) -> f32 {
    return length(p) - 1.0;
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let camera_pos = vec3( 0.0,  0.0, 0.9 + 2.0 * globals.mouse.y / globals.resolution.y);
    ...
    // from origin to the screen
    var ray = (camera_side * pos.x + camera_up * pos.y + camera_dir * target_depth);

    // starting point is camera_pos.
    var r = camera_pos;

    ray = normalize(ray);

    var color = vec3(0.0);
    let light_pos = vec3(2.0);
    for (var i = 0; i < 50; i++) {
        // if the ray reacyhes near enough to the surface, stop there
        if sdf(r) < 0.001 {
            // if the camera is inside the sphere
            if sdf(r) < -0.001 {
                color = vec3(0.78, 0.04, 0.03);
            } else {
                let ambient_light = 0.1;
                let diff = 0.9 * max(dot(normalize(light_pos - r), grad_sdf(r)), 0.0);
                color = vec3(0.0, 1.0, 1.0) * (diff + ambient_light);
            }
            break;
        }
