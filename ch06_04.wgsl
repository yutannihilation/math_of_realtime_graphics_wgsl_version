...
fn fdist34(p: vec3<f32>) -> vec4<f32> {
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    pos = pow(0.1 + globals.time, 1.1) * pos + 0.4 * globals.time;

    let f = fdist34(vec3(pos, globals.time));

    let wt1 = vec4( 0.5, -1.0,  1.4, -0.1);
    let wt2 = vec4(-0.3, -0.5, -1.2,  1.1);

    // let wt = mix(wt1, wt2, 0.5 + 0.5 * vec4(sin(globals.time / 4.0), sin(globals.time / 8.0), sin(globals.time / 2.0), sin(globals.time)));
    return vec4(vec3(abs(dot(f, wt2))), 1.0);
}
